...
// 实现原理：RAII 机制严重依赖于 C++ 的哪两个语言特性？
// 1. 栈对象自动析构：当栈帧销毁（函数返回、异常发生）时，定义在栈上的对象会自动调用析构函数。
// 2. 析构函数机制：C++ 保证对象销毁时一定会执行析构函数。

// 应用：智能指针，STL容器，文件操作(std::fstream)，线程锁(std::lock_guard/std::unique_lock)等
